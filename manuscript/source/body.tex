...
Quality   &   \\ \midrule
Functionality  & the ability of a system to not enter a failure mode, to be able to execute tasks required of it without fault (competence, ability, expertise), to achieve a goal state (liveness), and do so within permitted use of resources (time, materials, energy)\\
Compatibility  & degree to which the system can exchange information with (or be transferred to) other systems, users and infrastructure, and ability to share the same environment with other autonomous or sentient agents
Usability &  extent to which the system can be used to achieve specified goals with effectiveness (accessible, usable), efficiency (available, responsive) and satisfaction (satisfying) in a specified context\\
Reliability &degree to which the system performs specified functions under specified conditions for a specified period of time, dependable, consistency
Security &   1. protection against intentional subversion or forced failure, malicious access, use, modification, destruction, or disclosure 2. defining, achieving, and maintaining confidentiality, integrity, availability, non‐repudiation, accountability, authenticity, and reliability of a system 3. degree to which the system protects information and data so that users of the system have the degree of data access appropriate to their types and levels of authorization, confidentiality, integrity and accountability\\
Robustness & ease with which a software system or component can overcome adverse conditions, be maintained, modified to change or add capabilities or expand to new scales, correct faults or defects, improve performance or other attributes\\
Ethical &beneficence (doing good), non-maleficence (not harming), autonomy (preserving human decision making), justice (fair and reasonable), and explicability (easily understood)\\
Regulatability & ease in which the system is readable, explainable, transparent and understandable. System state information is presented in a conducive manner to support regulation and/or is readily comparable to specifications providing evidence of correct operation and supporting appropriate metrics.\\

\bottomrule
\end{tabular}
\caption{Trustworthiness qualities ontology}
\label{tab:ontology}
\end{table*}


avizienis2004basic a set of general concepts are required for dependable and secure computing, which may cover a wide range of applications and system failures, which comprise; availability (readiness for correct service), reliability (continuity of correct service), safety (absence of catastrophic consequences on the user(s) and the environment), integrity (absence of improper system alterations) and maintainability (ability to undergo modifications and repairs). 

Thiebes et al. argue for five foundational principles of trustworthy AS: beneficence (doing good), non-maleficence (not harming), autonomy (preserving human decision making), justice (fair and reasonable), and explicability (easily understood)~\cite{Thiebes2021}. These are based on and related to other discussion on ethically principled foundations of trustworthiness, Floridi 2018~\cite{Floridi2018}, and should be weighted appropriate to the strong international contribution to this area Jobin 2019~\cite{jobin2019global} but fail to capture the full trustworthiness spectrum of qualities presented here.
